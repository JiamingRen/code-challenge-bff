...
describe("Checkout", () => {
    ...
    describe("total method", () => {
        ...
        it("should throw error for item without pricing rule", () => {
            ...
            expect(() => checkout.total()).toThrow("Failed to scan ipd: No pricing rule found");
        });
        ...
        describe("buyXGetYFree pricing rule", () => {
            it("should apply 3-for-2 Apple TV deal correctly", () => {
                ...
                checkout.scan("atv");
                checkout.scan("atv");
                expect(checkout.total()).toBe(219.00); // 2 * $109.50
                ...
                checkout2.scan("atv");
                checkout2.scan("atv");
                expect(checkout2.total()).toBe(219.00); // 2 * $109.50 (1 free)
                ...
                checkout3.scan("atv");
                checkout3.scan("atv");
                expect(checkout3.total()).toBe(328.50); // 3 * $109.50
            });
        });
        ...
        describe("fullPrice pricing rule", () => {
            ...
            it("should charge full price for VGA adapter", () => {
                ...
                checkout.scan("vga");
                checkout.scan("vga");
                expect(checkout.total()).toBe(90.00); // 3 * $30.00
            });
        });
        ...
        describe("edge cases", () => {
            it("should handle multiple calls to total() correctly", () => {
                ...
                expect(firstTotal).toBe(139.50); // $109.50 + $30.00
            });
            ...
            it("should allow continuing to scan after calling total()", () => {
                ...
                const firstTotal = checkout.total();
                expect(firstTotal).toBe(109.50);
                
                checkout.scan("atv");
                const secondTotal = checkout.total();
                expect(secondTotal).toBe(219.00); // 2 * $109.50
            });
            ...
            it("should handle large quantities correctly", () => {
                ...
                expect(checkout.total()).toBe(3000.00); // 100 * $30.00
            });
        });
    });
});