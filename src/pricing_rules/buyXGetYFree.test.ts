...
describe("buyXGetYFree", () => {
  ...
  describe("validation tests", () => {
    it("should throw error when free >= buy", () => {
      expect(() =>
        buyXGetYFree({ product: testProduct, buy: 2, free: 2, quantity: 5 })
      ).toThrow("Free must be less than buy");
    });

    it("should throw error when price <= 0", () => {
      const invalidProduct = { ...testProduct, price: 0 };
      expect(() =>
        buyXGetYFree({ product: invalidProduct, buy: 3, free: 1, quantity: 5 })
      ).toThrow("Product price must be greater than 0");
    });

    it("should throw error when quantity <= 0", () => {
      expect(() =>
        buyXGetYFree({ product: testProduct, buy: 3, free: 1, quantity: 0 })
      ).toThrow("Quantity must be greater than 0");
    });

    it("should throw error when buy <= 0", () => {
      expect(() =>
        buyXGetYFree({ product: testProduct, buy: 0, free: 1, quantity: 5 })
      ).toThrow("Free must be less than buy");
    });

    it("should throw error when free <= 0", () => {
      expect(() =>
        buyXGetYFree({ product: testProduct, buy: 3, free: 0, quantity: 5 })
      ).toThrow("Free must be greater than 0");
    });
  });
});
